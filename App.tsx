...
import { Plus, LayoutDashboard, GraduationCap, Sparkles, X, Menu, Terminal } from 'lucide-react';
...
function App() {
  ...
  const handleAddSubject = (e: React.FormEvent) => {
    ...
    if (newSubject.name && newSubject.targetScore) {
      ...
      setNewSubject({ name: '', code: '', targetScore: 90, credits: 3, color: '#FF4B4B' });
    }
  };
    ...
    <div className="flex min-h-screen bg-[#F3F4F6] text-gray-900 font-sans">
      ...
      `}>
        <div className="p-6 flex items-center border-b border-gray-100">
          <Terminal className="w-8 h-8 text-[#FF4B4B] mr-3" />
          <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF4B4B] to-[#FFD43B]">
            PyTracker
          </span>
        </div>
        ...
        <nav className="p-4 space-y-1">
          <button 
            ...
            className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors ${view === 'dashboard' ? 'bg-red-50 text-[#FF4B4B]' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          {subjects.map(sub => (
            <button
              ...
              className={`w-full flex items-center px-4 py-2.5 text-sm font-medium rounded-lg transition-colors group ${selectedSubjectId === sub.id && view === 'subject_detail' ? 'bg-red-50 text-[#FF4B4B]' : 'text-gray-600 hover:bg-gray-50'}`}
            >
          ...
          <button
             ...
             className="w-full flex items-center px-4 py-2.5 text-sm font-medium text-gray-500 hover:text-[#FF4B4B] hover:bg-red-50 rounded-lg transition-colors mt-2 border border-dashed border-gray-200"
          >
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 border-t border-gray-100 bg-gray-50/50">
          {process.env.API_KEY && (
             <button
              ...
              className="w-full flex items-center justify-center px-4 py-2.5 bg-gradient-to-r from-[#FF4B4B] to-[#FF8C00] text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-lg transition-all active:scale-95"
            >
      ...
      <main className="flex-1 overflow-y-auto h-screen">
        ...
        <header className="lg:hidden bg-white border-b border-gray-200 p-4 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center">
            ...
            <span className="font-bold text-gray-900">PyTracker</span>
          </div>
        </header>
          ...
          {view === 'dashboard' ? (
            <div className="space-y-8 animate-fade-in">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Learning Dashboard</h1>
                  <p className="text-gray-500 mt-1">Track your Python & Streamlit mastery.</p>
                </div>
                <button 
                  onClick={() => setIsAddSubjectModalOpen(true)}
                  className="bg-[#FF4B4B] text-white px-5 py-2.5 rounded-lg font-medium shadow-md hover:bg-red-600 transition-all flex items-center self-start"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Terminal className="w-16 h-16 text-[#FF4B4B]" />
                  </div>
                  ...
                  <p className="text-xs text-[#FF4B4B] mt-2 font-medium flex items-center">
                     Across {subjects.length} modules
                  </p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                   <p className="text-sm text-gray-500 mb-1">Total Tasks Completed</p>
                   <h3 className="text-3xl font-bold text-gray-900">
                     {subjects.reduce((acc, s) => acc + s.scores.length, 0)}
                   </h3>
                   <p className="text-xs text-blue-600 mt-2 font-medium">Commits & Quizzes</p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                   <p className="text-sm text-gray-500 mb-1">Top Subject</p>
              ...
              {/* Subjects Grid */}
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Your Curriculum</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  ...
                  <button 
                    onClick={() => setIsAddSubjectModalOpen(true)}
                    className="flex flex-col items-center justify-center h-full min-h-[240px] rounded-xl border-2 border-dashed border-gray-300 text-gray-400 hover:border-[#FF4B4B] hover:text-[#FF4B4B] hover:bg-red-50 transition-all group"
                  >
                    <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center mb-3 group-hover:bg-red-100 transition-colors">
            ...
            <form onSubmit={handleAddSubject} className="p-6 space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  required
                  placeholder="e.g. Advanced Decorators"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FF4B4B] focus:border-[#FF4B4B] outline-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   ...
                   <input 
                     type="text" 
                     placeholder="PY301"
                     className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FF4B4B] outline-none"
              ...
              <div>
                ...
                <div className="flex items-center">
                  <input 
                    type="range" 
                    min="50" max="100" 
                    className="w-full mr-3 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#FF4B4B]"
              ...
              <div>
                 ...
                 <div className="flex space-x-3 flex-wrap gap-y-2">
                   {['#FF4B4B', '#306998', '#FFD43B', '#150458', '#4B8BBE', '#FFE873', '#646464', '#F0F2F6', '#0E1117', '#262730'].map(color => (
                     <button
                       key={color}
                       type="button"
              ...
              <div className="pt-2">
                <button type="submit" className="w-full py-2.5 bg-[#FF4B4B] text-white rounded-lg hover:bg-red-600 font-medium shadow-md">
                  Create Module
                </button>
              </div>
            </form>
      ...
      {isAIModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col animate-fade-in-up border border-gray-100">
             <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gradient-to-r from-red-50 to-white">
                <div className="flex items-center space-x-2">
                  <Sparkles className="w-5 h-5 text-[#FF4B4B]" />
                  <h3 className="font-bold text-gray-900 text-lg">AI Code Assistant</h3>
                </div>
             ...
             <div className="flex-1 overflow-y-auto p-8">
               {aiLoading ? (
                 <div className="flex flex-col items-center justify-center py-12 space-y-4">
                   <div className="w-12 h-12 border-4 border-red-200 border-t-[#FF4B4B] rounded-full animate-spin"></div>
                   <p className="text-gray-500 font-medium animate-pulse">Analyzing your code metrics...</p>
                 </div>
               ) : aiError ? (
                 ...
                 <div className="space-y-8">
                   <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 font-mono text-sm">
                     <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Execution Summary</h4>
                     ...
                     <div>
                       ...
                       <ul className="space-y-2">
                         {aiData.strengths.map((s, i) => (
                           <li key={i} className="flex items-start text-sm text-gray-700 bg-green-50/50 p-3 rounded-lg border border-green-100">
                     ...
                     <div>
                       ...
                       <ul className="space-y-2">
                         {aiData.weaknesses.map((w, i) => (
                           <li key={i} className="flex items-start text-sm text-gray-700 bg-amber-50/50 p-3 rounded-lg border border-amber-100">
                   ...
                   <div>
                     <h4 className="font-bold text-gray-900 mb-4 border-b pb-2">Optimization Tips & Snippets</h4>
                     <div className="space-y-4">
                       {aiData.tips.map((tip, i) => (
                         <div key={i} className="flex flex-col bg-[#262730] text-gray-200 p-4 rounded-xl border border-gray-700 shadow-md">
                           <div className="flex items-start mb-2">
                              <div className="bg-[#FF4B4B] text-white w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold mr-3 mt-0.5">
                                {i + 1}
                              </div>
